...
def inject_global_rules() -> str:
    ...
    This function reads both global memory and project-specific memory,
    then formats them for inclusion in the systemMessage. This ensures:
    1. All projects have access to global error prevention rules
    2. Project-specific rules are merged with global rules
    3. New projects automatically get global rules

    Returns:
        Formatted string containing global and project-specific rules
    """
    import re
    ...
    if global_memory_path.exists():
        try:
            ...
            # Extract essential rules only (ERR-001~ERR-016: Claude Code working errors)
            # These are the most frequently encountered errors across all projects
            essential_rules = []
            ...
            for i, line in enumerate(lines):
                # Check if we're in the essential rules section
                ...
                if in_essential_section:
                    ...
                    # Limit to ERR-001 through ERR-016 (Claude Code working errors)
                    if "ERR-017:" in line:
                        essential_rules = essential_rules[:-1]  # Remove the ERR-017 line
                        break

            if essential_rules:
                parts.append("\n## 🌍 GLOBAL RULES (Auto-loaded from ~/.claude/memory.md)")
                parts.append("## Common Errors Across All Projects (Claude Code Working)")
                parts.extend(essential_rules[:300])  # Limit to ~300 lines for token efficiency
    ...
    if project_memory_path.exists():
        try:
            ...
            parts.append(project_content[:500])  # Limit to ~500 chars
        except (OSError, UnicodeDecodeError):
            pass

    # Also check for LESSONS_LEARNED.md
    ...
    if lessons_path.exists():
        try:
            ...
            # Extract ERR- entries from lessons
            err_entries = re.findall(r"### ERR-\d+:.*?(?=### ERR-\d+:|##|\Z)", lessons_content, re.DOTALL)
            if err_entries:
                parts.append("\n## 📋 Project Lessons Learned")
                for entry in err_entries[:3]:  # Limit to 3 most recent
...
def get_global_memory_summary() -> str:
    ...
    Reads global memory file and returns key information for the session.
    This ensures all projects have access to global error prevention rules.

    Returns:
        Formatted summary string of global memory
    """
    global_memory_path = Path("C:/Users/drake.lee/.claude/memory.md")
    global_guide_path = Path("D:/GLOBAL_RULES_GUIDE.md")
    ...
    if global_memory_path.exists():
        try:
            ...
            err_count = len(set(err_matches))  # Unique errors
...
def format_session_output() -> str:
    ...
    # Add global memory information (Phase 3: Auto-loading global memory)
...
def main() -> None:
    ...
    def execute_session_start():
        ...
        # Inject global rules into session context (Option E implementation)
        global_rules = inject_global_rules()

        # Combine session output with global rules
        # Global rules are included in systemMessage so Claude can reference them
